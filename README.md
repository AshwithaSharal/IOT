...
for (int i=0; i<pinsCount; i=i+1)<br>
  { // counting the variable i from 0 to 9 <br>
  ...
  for (int i=pinsCount-1; i>0; i=i-1)<br>
  { // chasing left (except the outer leds) <br>
  digitalWrite(pins[i], HIGH); // switching the LED at index i on <br>
  delay(100); // stopping the program for 100 milliseconds <br>
  digitalWrite(pins[i], LOW); // switching the LED at index i off<br><br>
} <br>
}<br>
...
https://wokwi.com/projects/342592140210078291 -  Electronic Safe, powered by an Arduino Uno.
https://wokwi.com/projects/309829489359061570 -  Arduino LED Graph Bar - Move the potentiometer knob to control the LEDs Reference
